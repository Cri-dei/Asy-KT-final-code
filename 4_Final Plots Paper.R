...
ggplot() +geom_point(
  aes(x = POT_KT_12, y = POT_KT_21, col= Type),shape=19,alpha = 1/3,
  data = Sub)+
 # geom_abline()+
...
#  scale_color_manual(values = c("#00AFBB", "#E7B800", "#FC4E07"))+ 
  scale_color_manual(values = c("#99FF00", "#FF9933", "#66CCFF"))+ 
  xlab(TeX("Asymmetric tail Kendall's $\\tau_{XY}"))+
  ylab(TeX("Asymmetric tail Kendall's $\\tau_{YX}"))


ggsave("KT_selectedbasin2.jpeg", units="in",dpi=400, height=5.3,width =8)
...
ggplot()+ geom_point(aes(x=POT_KT_21, y=POT_KT_12, col=factor(HA)), size=3,data= Sub[which( Sub$Direction==12),])+ 
  ...
  ylab(TeX("Asymmetric tail Kendall's $\\tau_{XY}"))+ 
  xlab(TeX("Asymmetric tail Kendall's $\\tau_{YX}"))+
  labs(colour="Catchment ID")
...
Difference_plot<-Basin_map+ geom_segment(
  ...
  geom_segment(
    ...
    arrow = arrow(length = unit(0.30, "cm")))+
  scale_color_gradientn(colours = rev(rainbow(5)))+xlab("Easting 1")+ylab("Northing 1")+xlab("")+ylab("")
...
Real_flow<-Basin_map+ geom_segment(
  ...
  geom_segment(
    ...
    arrow = arrow(length = unit(0.30, "cm")))+
  scale_color_gradientn(colours = rev(rainbow(5)))+xlab("Easting 2")+ylab("Northing 2")+xlab("")+ylab("")
