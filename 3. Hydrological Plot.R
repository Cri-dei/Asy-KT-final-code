...
ggplot()+ geom_point(aes(x=POT_KT_21, y=POT_KT_12, col=factor(HA_1)), size=3,data= Sub[which( Sub$Direction==12),])+ 
  geom_point(aes(x=POT_KT_12, y=POT_KT_21, col=factor(HA_1)), size=3,data= Sub[which( Sub$Direction==21),])+
  theme_classic()+ geom_abline(intercept = 0, slope = 1)+ xlab("Kendall's Tau Downstream -> Upstream")+
  ylab("Kendall's Tau Upstream -> Downstream")
...
Matrix_basin<-Matrix_basin_proc
