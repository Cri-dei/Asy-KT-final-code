...
sum_melt$variable<-rep(c("KTXY","KTYX","KTSym"),6)
...
#dfmelt <- dfmelt %>% 
#  mutate(variable = str_replace(variable, "KTXY", "KT12"))%>% 
#  mutate(variable = str_replace(variable, "KTYX", "KT21"))
...
box22<-  dfmelt %>% 
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"))+xlab("")+ylab("")+ 
  theme(legend.position="bottom") 

box22
ggsave(paste0("0_CausalBoxplot_tst",par_tst,"Ccoef",par_C,"V2.jpeg"), units="in",dpi=400, height=8,width =13)
